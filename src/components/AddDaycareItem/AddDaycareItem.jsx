...
function AddDaycareItem({}){
    ...
    function submitHandler(event) {
        ...
        if (formInputs.daycareName.length ===0 ||formInputs.streetAddress.length ===0 || formInputs.city.length ===0 ||formInputs.region.length ===0||
            formInputs.postalcode.length ===0||formInputs.country.length ===0 || formInputs.agerange.length ===0||formInputs.latitude.length ===0|| formInputs.longitude.length ===0||
            formInputs.phoneNumber.length ===0 ||formInputs.email.length ===0)
            {
                setError (true);
        }
        else{
        ...
        <form className="daycare-form__form" name="addDaycareForm" id="addDaycareForm" onSubmit={submitHandler}>
            ...
            <fieldset className="daycare-form__details-container" form="addDaycareForm" name="daycareDetailsFields" >
                ...
                <input className= "daycare-form__input" type="text" name="daycareName" id="daycareName" placeholder="Daycare Name"    value={formInputs.daycareName}
                          ...
                          { error && formInputs.daycareName.length<=0?  <label className="daycare-form__label-error">Daycare Name can't be empty</label>:""}
...
{ error && formInputs.streetAddress.length<=0?  <label className="daycare-form__label-error">Daycare Street address can't be empty</label>:""}
                ...
                <input className=  "daycare-form__input" type="text" name="city" id="city" placeholder="City"      value={formInputs.city}
                            onChange={handleChange}/>
 { error && formInputs.city.length<=0?  <label className="daycare-form__label-error">City can't be empty</label>:""}
                ...
                <input className= "daycare-form__input" type="text" name="region" id="region" placeholder="Region"  value={formInputs.region}
                            onChange={handleChange} />
                             { error && formInputs.region.length<=0?  <label className="daycare-form__label-error">Region can't be empty</label>:""}
               <label className="daycare-form__label" htmlFor="country">Country</label>
                <input className= "daycare-form__input" type="text" name="country" id="country" placeholder="Country"     value={formInputs.country}
                            onChange={handleChange} />
 { error && formInputs.country.length<=0?  <label className="daycare-form__label-error">Country can't be empty</label>:""}
...
{ error && formInputs.postalcode.length<=0?  <label className="daycare-form__label-error">Postal code can't be empty</label>:""}
                ...
                <input className= "daycare-form__input" type="text" name="latitude" id="latitude" placeholder="Latitude"  value={formInputs.latitude}
                            onChange={handleChange} />
                               { error && formInputs.latitude.length<=0?  <label className="daycare-form__label-error">Latitude can't be empty</label>:""}
                ...
                <input className= "daycare-form__input" type="text" name="longitude" id="longitude" placeholder="Longitude"   value={formInputs.longitude}
                            onChange={handleChange} />
                               { error && formInputs.longitude.length<=0?  <label className="daycare-form__label-error">Longitude can't be empty</label>:""}
            </fieldset>
            <fieldset className="daycare-form__contact-container" form="addDaycareForm" name="contactDetailsFields">
                ...
                <input className= "daycare-form__input" type="text" name="phoneNumber" id="phoneNumber" placeholder="Phone Number"    value={formInputs.phoneNumber}
                            onChange={handleChange}/>
{ error && formInputs.phoneNumber.length<=0?  <label className="daycare-form__label-error">Phone Number can't be empty</label>:""}
                ...
                <input className= "daycare-form__input" type="text" name="email" id="email" placeholder="Email"      value={formInputs.email}
                            onChange={handleChange}/>
{ error && formInputs.email.length<=0?  <label className="daycare-form__label-error">Email can't be empty</label>:""}
...
{ error && formInputs.agerange.length<=0?  <label className="daycare-form__label-error">Age range can't be empty</label>:""}
